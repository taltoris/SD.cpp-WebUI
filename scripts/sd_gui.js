...
function initializeDOMElements() {
    ...
    elements = {
        ...
        img2imgNegativePromptGroup: document.getElementById('img2imgNegativePromptGroup'),
...
function applyDefaults() {
    ...
    // Strength
    if (elements.strength && defaults.strength !== undefined) {
        elements.strength.value = defaults.strength;
        if (elements.strengthValue) {
            elements.strengthValue.textContent = defaults.strength;
        }
    }
...
function updateVisibility(params, defaults) {
    ...
    if (elements.img2imgNegativePromptGroup) {
        if (hideNegPrompt) {
            elements.img2imgNegativePromptGroup.classList.add('hidden');
        } else {
            elements.img2imgNegativePromptGroup.classList.remove('hidden');
        }
    }
...
function setupEventListeners() {
    ...
    if (elements.img2imgGenerateBtn) {
        elements.img2imgGenerateBtn.addEventListener('click', function() {
            generateImage('img2img');
        });
    }
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (e.ctrlKey && e.key === 'Enter') {
            ...
            if (activeTab) {
                ...
                } else if (activeTab.id === 'img2img-tab' && elements.img2imgGenerateBtn) {
...
function buildGenerationParams(mode) {
    var promptEl = mode === 'img2img' ? elements.img2imgPrompt : elements.prompt;
    var negPromptEl = mode === 'img2img' ? elements.img2imgNegativePrompt : elements.negativePrompt;
    
    var params = {
        prompt: promptEl ? promptEl.value : '',
        negative_prompt: negPromptEl ? negPromptEl.value : '',
    ...
    if (mode === 'img2img' && elements.strength) {
...
async function generateImage(mode) {
    ...
    if (mode === 'img2img') {
        ...
        if (!fileInput || !fileInput.files || !fileInput.files[0]) {
            showMessage('Please select an input image', 'error');
            return;
        }
    }
    
    var generateBtn = mode === 'img2img' ? elements.img2imgGenerateBtn : elements.generateBtn;
    var loadingIndicator = mode === 'img2img' ? elements.img2imgLoadingIndicator : elements.loadingIndicator;
    var outputContainer = mode === 'img2img' ? elements.img2imgOutputContainer : elements.outputContainer;
    
    if (generateBtn) generateBtn.disabled = true;
    if (loadingIndicator) loadingIndicator.classList.add('active');
    if (outputContainer) outputContainer.innerHTML = '';
    
    var params = buildGenerationParams(mode);
    
    console.log('Generating image with params:', params);
    
    try {
        if (mode === 'img2img' && elements.initImage.files[0]) {
            params.init_image = await fileToBase64(elements.initImage.files[0]);
        }
        
        var endpoint = serverOnline && modelLoaded ? 
            (mode === 'img2img' ? '/img2img' : '/generate') :
            (mode === 'img2img' ? '/img2img_cli' : '/generate_cli');
